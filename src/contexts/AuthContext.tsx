...
const parseSuperAdminEmails = (): Set<string> => {
  const raw = import.meta.env.VITE_SUPERADMIN_EMAILS?.trim();
...
const resolveRole = (u: any): UserRole => {
  const email = (u.email || "").trim().toLowerCase();
  if (parseSuperAdminEmails().has(email)) return "superadmin";
...
const toUser = (u: any): User => ({
  ...
  email: u.email,
  role: resolveRole(u),
  tenantId: u.tenant_id != null ? String(u.tenant_id) : u.tenant?.id != null ? String(u.tenant.id) : undefined,
  tenantName: u.tenant_name ?? u.tenant?.name ?? undefined,
  avatar: u.avatar,
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  const [user, setUser] = useState<User | null>(() => (isFirebaseConfigured ? null : loadUser()));
