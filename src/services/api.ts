...
export const authAPI = {
  ...
  createAdmin: (name: string, email: string, password: string, tenantName: string) =>
    apiRequest('/auth/superadmin/admin', {
      method: 'POST',
      body: JSON.stringify({ name, email, password, tenant_name: tenantName }),
    }),

  listAdmins: (params?: { page?: number; limit?: number; search?: string; tenant_id?: number }) => {
    const searchParams = new URLSearchParams();
    if (params?.page) searchParams.append('page', String(params.page));
    if (params?.limit) searchParams.append('limit', String(params.limit));
    if (params?.search) searchParams.append('search', params.search);
    if (params?.tenant_id != null) searchParams.append('tenant_id', String(params.tenant_id));
  ...
  listSuperAdminStudents: (params?: { tenant_id?: number; search?: string; page?: number; limit?: number }) => {
    const searchParams = new URLSearchParams();
    if (params?.tenant_id != null) searchParams.append('tenant_id', String(params.tenant_id));
    if (params?.search) searchParams.append('search', params.search);
    if (params?.page) searchParams.append('page', String(params.page));
    if (params?.limit) searchParams.append('limit', String(params.limit));
    const qs = searchParams.toString();
    return apiRequest(`/auth/superadmin/students${qs ? `?${qs}` : ''}`);
  },

  listSuperAdminInstructors: (params?: { tenant_id?: number; search?: string; page?: number; limit?: number }) => {
    const searchParams = new URLSearchParams();
    if (params?.tenant_id != null) searchParams.append('tenant_id', String(params.tenant_id));
    if (params?.search) searchParams.append('search', params.search);
    if (params?.page) searchParams.append('page', String(params.page));
    if (params?.limit) searchParams.append('limit', String(params.limit));
    const qs = searchParams.toString();
    return apiRequest(`/auth/superadmin/instructors${qs ? `?${qs}` : ''}`);
  },
...
export const aiAPI = {
  ...
  /**
   * Career roadmap AI — rank selected courses and suggest study order
   * @param courseIds Real course IDs from the catalog (1–20)
   */
  recommendRoadmap: (courseIds: Array<number | string>) => {
    const ids = courseIds
      .map((id) => (typeof id === 'number' ? id : parseInt(String(id), 10)))
      .filter((n) => Number.isFinite(n) && n > 0);
    return apiRequest('/ai/roadmap/recommend', {
      method: 'POST',
      body: JSON.stringify({ courseIds: ids }),
    }).then((res) => {
      const data = res?.data ?? res;
      return { ...data, success: res?.success ?? true };
    });
  },
  ...
  getSummaries: (limit: number = 20, offset: number = 0) =>
    apiRequest(`/ai/summaries?limit=${limit}&offset=${offset}`).then((res) => {
      ...
      const data = res?.data ?? res;
      const pagination = res?.pagination || { limit, offset, total: 0, hasMore: false };
